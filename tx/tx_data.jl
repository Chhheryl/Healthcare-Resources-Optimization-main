...
projectbasepath = "../"
include(joinpath(projectbasepath, "processing/LocalDataCommon.jl"))
include(joinpath(projectbasepath, "processing/GeographicData.jl"))
...
function load_data_tx(
	...
	rawdata = DataFrame(CSV.File(joinpath(projectbasepath, "TX_data/tx_tsa_hospitalizations.csv")))
...
function load_beds_tx(data; bedtype=:all, use_rounding=true)
	counties_list = JSON.parsefile(normpath(projectbasepath, "TX_data/tsa_counties.json"));
	county_fips = [c["county_fips"] for c in counties_list]
	county_fips = parse.(Int, county_fips)

	beds_data = DataFrame(CSV.File(joinpath(projectbasepath, "TX_data/definitivehc.csv")))
