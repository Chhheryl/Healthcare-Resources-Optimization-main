...
function load_nurse_employment_data()
	nurse_data_employment = DataFrame(CSV.File("../data/nurses/deaggregated_by_hospital_beds.csv"), copycols=true)
...
function load_nurse_beds_data()
	nurse_data_beds = DataFrame(CSV.File("../data/hospitals/definitivehc.csv"), copycols=true)
...
function num_nurses_from_beds(
	...
	nurse_data_local.est_nurses_per_shift =
		  (nurses_per_bed_icu * nurse_data_local.NUM_ICU_BEDS_sum) + (nurses_per_bed_regular * nurse_data_local.non_icu_beds_sum)
...
function load_nurse_ahrf_data()
	nurse_data = DataFrame(CSV.File("../data/nurses/nurses_per_county.csv"))
